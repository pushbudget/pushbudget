angular.module('pushbudget').service('userProfile', function ($http, $q, absoluteUrl) {
  ...
  this.removeInstitution = function (accessToken) {
    ...
    $http({
      method: 'POST',
      url: absoluteUrl.url + 'api/split/' + transaction.transId,
      data: transaction
   ...
   this.addInstitution = function (userId, publicToken) {
    ...
    $http({
      method: 'POST',
      url: absoluteUrl.url + 'api/authenticate/' + userId,
    ...
    this.refreshInstitution = function (userId, accessToken) {
    	...
    	//get user's publictoken given the userId and accesstoken (which is unique to the institution for the user)
		    ...
		    $http({
		      method: 'GET',
		      url: absoluteUrl.url + 'api/token/' + accessToken + '/user/' + userId
    ...
    this.updateUserPublicTokenUponRefresh = function (userId, accessToken, newPublicToken ) {
    	...
    	//get user's publictoken given the userId and accesstoken (which is unique to the institution for the user)
		    ...
		    $http({
		      ...
		      method: 'PATCH',
      			url: absoluteUrl.url + 'api/authenticate/' + userId + '/refresh/' + accessToken,
	...
	this.deleteInstitution = function (userId, accessToken ) {
    	...
    	//get user's publictoken given the userId and accesstoken (which is unique to the institution for the user)
		    ...
		    $http({
		      ...
		      method: 'DELETE',
      			url: absoluteUrl.url + 'api/user/plaid/' + accessToken + '/' + userId      			
